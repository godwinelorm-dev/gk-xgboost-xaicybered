...
CUT_LOW = 22   # Secure-score >= 22 -> Low Risk
CUT_MED = 12   # 12..21 -> Medium Risk; < 12 -> High Risk
...
def load_data(path=DATA_PATH):
    ...
    y = np.where(secure_score >= CUT_LOW, 0, np.where(secure_score >= CUT_MED, 1, 2))
    X = pd.DataFrame()
...
def mcw_only_model(seed):
    """Ablation fourth cell: reduced min_child_weight, NO focal loss."""
    return XGBClassifier(**SHARED, random_state=seed, objective='multi:softmax',
                         num_class=N_CLASS, eval_metric='mlogloss',
                         min_child_weight=GK_MCW)


def evaluate(model_factory, X_raw, y, resample=None, weight=False, lgbm=False):
    """RobustScaler is fitted INSIDE each training fold only (no leakage)."""
    res = {m: [] for m in METRICS}
    for seed in SEEDS:
        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=seed)
        for tr, te in skf.split(X_raw, y):
            scaler = RobustScaler().fit(X_raw.iloc[tr])
            X_tr = pd.DataFrame(scaler.transform(X_raw.iloc[tr]), columns=X_raw.columns)
            X_te = pd.DataFrame(scaler.transform(X_raw.iloc[te]), columns=X_raw.columns)
            y_tr, y_te = y[tr], y[te]
            if lgbm:
...
def main():
    ...
    X, y = load_data()   # RAW features; scaling happens in-fold inside evaluate()
    ...
    res_base  = evaluate(baseline_model, X, y)
    res_gk    = evaluate(gk_model,       X, y)
    res_smote = evaluate(None, X, y, resample='smote')
    res_wt    = evaluate(None, X, y, weight=True)
    res_lgbm  = evaluate(None, X, y, lgbm=True)
    res_focal = evaluate(focal_only_model, X, y)
    res_mcw   = evaluate(mcw_only_model, X, y)   # fourth ablation cell
    ...
    print("\nABLATION (macro F1) - four cells:")
    print(f"  Baseline (focal off, mcw=8)   : {np.mean(res_base['f1']):.4f}")
    print(f"  mcw=3 only (focal off, mcw=3) : {np.mean(res_mcw['f1']):.4f}")
    print(f"  Focal-only (focal on, mcw=8)  : {np.mean(res_focal['f1']):.4f}")
    print(f"  Full GK-XGBoost (both)        : {np.mean(res_gk['f1']):.4f}")
